...
function InstallCmd({ label, command }: { label: string; command: string }) {
  ...
  return (
    <div
      onClick={handleCopy}
      className="bg-terminal/70 border-2 border-mustard py-2 px-6 font-terminal text-[0.9rem] text-terminal-green text-left flex justify-between items-center cursor-pointer transition-colors hover:border-terminal-green"
    >
      <div>
